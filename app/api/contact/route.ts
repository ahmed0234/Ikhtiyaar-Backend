...
const BUSINESS_EMAIL = process.env.BUSINESS_EMAIL ?? "hassankiller0234@gmail.com";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { error } = await resend.emails.send({
      from:     `${data.firstName} ${data.lastName} <support@ikhtiyaar.com>`,   // swap for your verified domain later
      to:       [BUSINESS_EMAIL],
      replyTo:  data.email,
